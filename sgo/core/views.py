...
from .models import Funcao, Setor, TipoRisco, Risco, Exame, Grupo, GrupoFuncao, GrupoRisco, GrupoExame, TipoExame, Treinamento, GrupoTreinamento
...
from .forms import GrupoRiscoForm, GrupoExameForm, TipoExameForm, TreinamentoForm,EmpregadoForm, GrupoTreinamentoForm, Empregado,CoordenadorForm
from .forms import AtendimentoForm, TrocaSenhaForm
...
@login_required
def home(request):
    ano5 = datetime.datetime.now().strftime ("%Y")
    qtd5 = Atendimento.objects.filter(data_atendimento__year=ano5).count() + 20
    ano4 = int(ano5) - 1
    qtd4 = Atendimento.objects.filter(data_atendimento__year=ano4).count() + 25
    ano3 = int(ano4) - 1
    qtd3 = Atendimento.objects.filter(data_atendimento__year=ano3).count() + 30
    ano2 = int(ano3) - 1
    qtd2 = Atendimento.objects.filter(data_atendimento__year=ano2).count() + 35
    ano1 = int(ano2) - 1
    qtd1 = Atendimento.objects.filter(data_atendimento__year=ano2).count() + 40
    
    anos = {'ano1': ano1, 'ano2': ano2, 'ano3': ano3, 'ano4': ano4, 'ano5': ano5}
    qtdes = {'qtd1': qtd1, 'qtd2': qtd2, 'qtd3': qtd3, 'qtd4': qtd4, 'qtd5': qtd5}
    return render(request, 'core/home.html', {'qtdes': qtdes, 'anos': anos})
@login_required
...
def grupoexame_novo(request, id):
    ...
    if request.method == "POST":
        ...
        count = GrupoExame.objects.filter(grupo_id=id, exame_id=exame_id).count()
        if count > 0:
            messages.error(request, 'Registro já cadastrado !')
...
def lista_treinamentos(request):
    ...
    data['form'] = form
    return render(request, 'core/lista_treinamentos.html', data)

@login_required
def treinamento_novo(request):
    if request.method == "POST":
        nome = request.POST.get('id_nome')
        validade = request.POST.get('validade')
        count = Treinamento.objects.filter(nome=nome).count()
        if count > 0:
            messages.error(request, 'Registro já cadastrado com este Nome !')
            return redirect('treinamento_novo')
        else:
        ...
        data = {}
        data['form'] = form
        return render(request, 'core/treinamento_novo.html', data)

@login_required
def treinamento_update(request, id):
    treinamento = Treinamento.objects.get(id=id)
    form = TreinamentoForm(request.POST or None, instance=treinamento)
    data = {}
    ...
    if request.method == "POST":
        ...
        form = TreinamentoForm
        return render(request, 'core/treinamento_update.html', data)

@login_required
def treinamento_delete(request, id):
    treinamento = Treinamento.objects.get(id=id)
    treinamento.delete()
    messages.success(request, 'Registro Excluido com sucesso !')
    return redirect('lista_treinamentos')

def grupotreinamento_novo(request, id):
    grupo = Grupo.objects.get(id=id)
    grupotreinamentos = GrupoTreinamento.objects.filter(grupo_id=id)
    treinamentos = Treinamento.objects.all().order_by('nome')
    data = {}
    data['grupo'] = grupo
    data['grupotreinamentos'] = grupotreinamentos
    data['treinamentos'] = treinamentos
    
    if request.method == "POST":
        treinamento_id = request.POST.get('treinamento_id')
       
        count = GrupoTreinamento.objects.filter(grupo_id=id, treinamento_id=treinamento_id).count()
        if count > 0:
            messages.error(request, 'Registro já cadastrado !')
            return redirect('/grupotreinamento_novo/' + str(id))
        else:
            g = GrupoTreinamento.objects.create(
                grupo_id = id,
        ...
        return render(request, 'core/grupotreinamento_novo.html', data)

@login_required
def grupotreinamento_delete(request, id):
    grupotreinamento = GrupoTreinamento.objects.get(id=id)
    grupo_id = grupotreinamento.grupo_id
    grupotreinamento.delete()
    return redirect('/grupotreinamento_novo/' + str(grupo_id))
